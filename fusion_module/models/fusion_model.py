class FusionModel:
    ...
    def predict(self, echo, ecg, clinical):
        """
        echo, ecg, clinical are already outputs from agents
        Example:
        {"level": "...", "score": 0.x}
        """
        ...
        # 🔹 Convert to numeric
        e = self.level_to_num(echo.get("level"))
        g = self.level_to_num(ecg.get("level"))
        c = self.level_to_num(clinical.get("level"))
        ...
        if e is not None:
            values.append(e * W_ECHO * echo.get("score", 0))
            weights.append(W_ECHO)

        if g is not None:
            values.append(g * W_ECG * ecg.get("score", 0))
            weights.append(W_ECG)

        if c is not None:
            values.append(c * W_CLINICAL * clinical.get("score", 0))
